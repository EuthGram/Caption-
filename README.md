...
```
index.html       — live status page served at your root URL (no more 404)
...
Visiting your Vercel URL in a browser (e.g. `https://your-project.vercel.app/`) now shows a small dark status page that pings `/api/webhook` live and reports whether the bot's backend is reachable — instead of a 404. The bot itself is still only used through Telegram; this page is just a health check.
